...
/**
 * Provide Perlin noise modulated by Voronoï diagram for make Continental-like heighmap.
...
public class ContinentalGenerator extends PlanetGenerator {
	...
	protected int[] generateHeightmap() {
		...
		final PerlinNoise perlin = new PerlinNoise(7, getWidth(), getHeight(), getInformations().getSeed() );
		...
		/* Permet de determiner oï¿½ s'arrette l'eau */
		int[] tab = new int[256];
		...
		if(MultiThreadUtil.PROCESSORS_COUNT>1) {
			...
			for(int x = 0; x < getWidth(); ++x) {
				for(int y = 0; y < getHeight(); ++y)  {
					int result = (int)(perlin.getPerlinPointAt(x, y) * 255* (1.0f - 0.8f*diagram.getValueAt(x, y)));
					//result += m_random.nextInt(2); //Ajout d'un pixel 1 ï¿½ la valeur du pixel 1 fois sur 2
					tab[result]++;
		...
		/* On lie le pixel seuillant l'eau au pourcentage de terre immergï¿½es */
	...
	private void addSource(int x, int y, Set<Integer> visited, int maxiteration) {
		...
		if(x==0) xm1 = m_widthm1;
		if(y==0) ym1 = m_widthm1;
	...
	public ContinentalGenerator(int texture_width, int texture_height,
			PlanetInformations info, TerrainPalette color_palette) {	
		super(texture_width, texture_height, info, color_palette);
		
		/* Crï¿½ation de l'objet hazard */
